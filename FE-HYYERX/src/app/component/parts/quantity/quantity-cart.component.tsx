...
const QuantityCart: FC<QuantityCartProps> = ({ quantity, itemProductCart }) => {
    ...
    const handleDecrement = () => {
        const objectDecremet = {
            type: 'DECREMENT',
            quantityOrder,
            itemProductCart
        }
        actions.updateQuantityCart(objectDecremet)
        if (quantityOrder > 1) {
    ...
    const handleIncrement = () => {
        const objectIncremet = {
            type: 'INCREMENT',
            quantityOrder,
            itemProductCart
        }
        actions.updateQuantityCart(objectIncremet)
        const requestQuantityCart = {
            productId: itemProductCart.product._id,
            quantity: quantityOrder + 1
        }
    ...
    const changeInputQuantity = (event: any) => {
        ...
        console.log(itemProductCart.product.quantity)
        updateProductToCart(requestQuantityCart)
    }
    ...
    return (
        <div>
            ...
            <div className='flex border border-solid border-gray-300 max-w-max rounded-lg'>
                <div
                    className='px-3 py-2 border-r border-solid border-gray-300 cursor-pointer hover:bg-red-200' onClick={handleDecrement}
                >
                    -
                </div>
                <input
                    type='text'
                    value={quantityOrder}
                    className='outline-none w-[30px] text-center focus:border-red-400'
                    onChange={changeInputQuantity}
                />
                <div
                    className='px-3 py-2 border-l border-solid border-gray-300 cursor-pointer hover:bg-red-200 ' onClick={handleIncrement}
                >
                    +
                </div>
            </div>
